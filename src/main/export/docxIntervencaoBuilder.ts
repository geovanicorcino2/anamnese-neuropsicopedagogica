...
function montarCorpoDocumento(conteudo: ConteudoIntervencao): string {
  const partes: string[] = [];
  ...
  partes.push(paragrafoItem("Data do atendimento", new Date(conteudo.geradoEm).toLocaleDateString("pt-BR")));
  partes.push(paragrafoItem("Profissional", `${conteudo.nomeProfissional} — ${conteudo.tituloProfissional}`));

  partes.push(tituloSecao("Tempo de sessão"));
  partes.push(paragrafo(conteudo.tempoSessao));

  partes.push(tituloSecao("Atividades"));
...
function montarDocumentXml(conteudo: ConteudoIntervencao): string {
