...
import { montarPromptSugestoes } from "@core/services/promptSugestoes";
...
export async function gerarSugestoesParaFicha(idFicha: string): Promise<SugestaoIA> {
  ...
  const texto = await gerarTextoIa(perfil, prompt);

  return sugestoesRepository.salvarSugestao(idFicha, texto);
}
